...
def read_float(client, address, slave):
    try:
        rr = client.read_holding_registers(address=address, count=2, device_id=slave)
        if rr.isError():
            if DEBUG_RAW_REGISTERS:
...
def read_int64(client, address, slave):
    try:
        rr = client.read_holding_registers(address=address, count=4, device_id=slave)
        if rr.isError():
            if DEBUG_RAW_REGISTERS:
                print("[{}] MODBUS READ ERROR at {}: {}".format(get_log_ts(), address, rr), flush=True)
            return None
...
def poll_meter():
    ...
    client = ModbusTcpClient(MODBUS_IP, port=MODBUS_PORT, timeout=1, framer=MODBUS_FRAMER)
